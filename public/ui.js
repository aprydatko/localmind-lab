...
export const renderUploadedFiles = (container, files = []) => {
  ...
  title.textContent = 'Files in docs/';
...
export const resultCard = (data, temperature, mode, streaming = false) => {
  const result = streaming
    ? { content: '', finishReason: 'streaming', usage: {}, reasoning: null }
    : data;
  const card = document.createElement('article');
  ...
  header.innerHTML = `<strong>temperature ${temperature}</strong><span>${result.finishReason}</span>`;
  ...
  renderMarkdown(answerDiv, result.content);

  // Add token usage
  const tokenUsageSection = document.createElement('section');
  tokenUsageSection.className = 'token-usage';
  tokenUsageSection.innerHTML = `
    <div>
      <span>INPUT / PROMPT</span>
      <strong>${result.usage?.prompt_tokens ?? '—'}</strong>
      <small>messages + system prompt + tools + previous results</small>
    </div>
    <div>
    ...
    <div>
      ...
      <small>input + output for all LLM calls</small>
    </div>
  `;
  card.appendChild(tokenUsageSection);
